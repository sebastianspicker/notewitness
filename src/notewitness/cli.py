...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.command == "metronome-plan":
        try:
            plan = MetronomePlan(
                bpm=args.bpm,
    ...
    if args.command == "transcription-plan":
        try:
            spec = TranscriptionJobSpec(
                job_id=args.job_id,
                spans=(
                    MediaSpan(
        ...
        output = {
            "job": spec.as_dict(),
            "export_losses": [
                asdict(loss) for loss in transcript_export_losses(spec)
            ],
    ...
    try:
        graph = EvidenceGraph.load(args.project)
        graph.require_valid()
    ...
    if args.command == "preview-relations":
        try:
        ...
        preview["rights_allow_remote"] = graph.selected_events_allow_remote(
            args.event_ids
        )
        print(json.dumps(preview, ensure_ascii=False, indent=2, sort_keys=True))
        return 0

    if args.command == "suggest-relations":
        try:
        ...
        print(
            json.dumps(
                result.as_safe_dict(), ensure_ascii=False, indent=2, sort_keys=True
            )
        )
        return 0
