...
def validate_rights(
    rights: RecordIndex, issues: list[ValidationIssue]
) -> None:
    for record_id, record in rights.items():
        ...
        for field in ("remote_processing", "model_training"):
            if not isinstance(record.get(field), bool):
                issues.append(
                    ValidationIssue(f"{path}.{field}", "must be boolean")
                )
        require_non_empty_string(
            record.get("retention"), f"{path}.retention", issues
        )
...
def validate_sources(
    ...
    for record_id, record in sources.items():
        ...
        require_fields(
            record, path, ("kind", "uri", "sha256", "rights_id"), issues
        )
...
def validate_actors(
    actors: RecordIndex, issues: list[ValidationIssue]
) -> None:
    for record_id, record in actors.items():
        ...
        if not isinstance(visibility, str) or visibility not in VISIBILITY_LEVELS:
            issues.append(
                ValidationIssue(f"{path}.visibility", "has an unknown tier")
            )
...
def validate_targets(
    ...
    for record_id, record in targets.items():
        ...
        require_fields(
            record, path, ("source_id", "selector", "alignment_state"), issues
        )
        validate_id(record.get("source_id"), f"{path}.source_id", issues)
        source_id = record.get("source_id")
        if not isinstance(source_id, str) or source_id not in sources:
            issues.append(
                ValidationIssue(f"{path}.source_id", "must reference a source")
            )
        ...
        if not isinstance(selector, dict):
            ...
            require_fields(
                selector,
                selector_path,
                ("stream_id", "start_us", "duration_us"),
                issues,
            )
            reject_unknown_fields(
                selector,
            ...
            if "spatial" in selector and not isinstance(
                ...
                issues.append(
                    ValidationIssue(
                        f"{selector_path}.spatial", "must be an object or null"
                    )
                )
        if "musical_selector" in record and not isinstance(
            ...
            issues.append(
                ValidationIssue(
                    f"{path}.musical_selector", "must be an object or null"
                )
            )
        alignment_state = record.get("alignment_state")
        if (
            not isinstance(alignment_state, str)
            or alignment_state not in ALIGNMENT_STATES
        ):
            issues.append(
                ValidationIssue(
                    f"{path}.alignment_state", "has an unsupported state"
                )
            )


def validate_generators(
    generators: RecordIndex, issues: list[ValidationIssue]
) -> None:
    for record_id, record in generators.items():
        ...
        if (
            not isinstance(generator_kind, str)
            or generator_kind not in GENERATOR_KINDS
        ):
        ...
        if record.get("kind") == "machine":
            ...
            require_non_empty_string(
                record.get("weight_hash_state"),
                f"{path}.weight_hash_state",
                issues,
            )
        for field in ("model", "weight_hash_state"):
            if field in record and not isinstance(record.get(field), str):
                issues.append(
                    ValidationIssue(f"{path}.{field}", "must be a string")
                )
